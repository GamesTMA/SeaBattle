...
module.exports = async (ctx, next) => {
  ...
  if (ctx.group) {
    ctx.group.username = ctx.chat.username
    ctx.group.lastMessage = Date.now()
    ctx.group.title = convertChars(ctx.chat.title)
    ctx.group.alive = true
  }
  ...
  if (ctx.user && ctx.from) {
    ...
    if (!ctx.group) ctx.user.alive = true
